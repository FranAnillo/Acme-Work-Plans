...
@Service
public class ManagWorkplanListService implements AbstractListService<Manag, Workplan> {
	
	@Autowired
	private ManagWorkplanRepository repository;
	...
	public List<Workplan> findMany(final Request<Workplan> request) {
		...
		for (final Workplan workplan:this.repository.findMany()) {
			if (workplan.getManag().equals(this.repository.findOneWorkplanById(workplanId).getManag())) {
				result.add(workplan);
			}
		}
